...
rc=$?; if [[ $rc != 0 ]]; then echo "Instance directory generation failed! exiting"; echo " - Solr         - Failed [Instance directory generation failed]" >> "$LOG_PATH"/SummaryReport.txt; exit $rc; fi

solrctl instancedir --create "$SOLR_INSTANTDIR_NAME" /tmp/solr.$$
rc=$?; if [[ $rc != 0 ]]; then echo "Config file creation failed! exiting"; echo " - Solr         - Failed [Config file creation failed]" >> "$LOG_PATH"/SummaryReport.txt; exit $rc; fi
...
rc=$?; if [[ $rc != 0 ]]; then echo "Collection creation failed! exiting"; echo " - Solr         - Failed [Collection creation failed]" >> "$LOG_PATH"/SummaryReport.txt; exit $rc; fi
...
#rc=$?; if [[ $rc != 0 ]]; then echo "java command failed! exiting"; echo " - Solr         -  Failed [java command failed]" >> "$LOG_PATH"/SummaryReport.txt; exit $rc; fi
...
rc=$?; if [[ $rc != 0 ]]; then echo "curl upload command failed! exiting"; echo " - Solr         -  Failed [curl upload command failed]" >> "$LOG_PATH"/SummaryReport.txt; exit $rc; fi
...
rc=$?; if [[ $rc != 0 ]]; then echo "curl query command failed! exiting"; echo " - Solr         -  Failed [curl query command failed]" >> "$LOG_PATH"/SummaryReport.txt; exit $rc; fi
