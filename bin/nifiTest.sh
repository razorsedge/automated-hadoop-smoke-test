...
rc=$?; if [[ $rc != 0 ]]; then echo "Unable to create temporary Nifi template! Exiting!";  echo " - Nifi         - Failed [Unable to instantiate template]" >> "$LOG_PATH"/SummaryReport.txt; exit $rc; fi
...
rc=$?; if [[ $rc != 0 ]]; then echo "Unable to create new process group! Exiting!";  echo " - Nifi         - Failed [Unable to instantiate template]" >> "$LOG_PATH"/SummaryReport.txt; exit $rc; fi
...
rc=$?; if [[ $rc != 0 ]]; then echo "Unable to upload template to Nifi! Exiting!";  echo " - Nifi         - Failed [Unable to upload template to Nifi]" >> "$LOG_PATH"/SummaryReport.txt; exit $rc; fi
...
rc=$?; if [[ $rc != 0 ]]; then echo "Unable to instantiate template! Exiting!";  echo " - Nifi         - Failed [Unable to instantiate template]" >> "$LOG_PATH"/SummaryReport.txt; exit $rc; fi
...
rc=$?; if [[ $rc != 0 ]]; then echo "Unable to start the process! Exiting!";  echo " - Nifi         - Failed [Unable to start the process]" >> "$LOG_PATH"/SummaryReport.txt; exit $rc; fi
...
rc=$?; if [[ $rc != 0 ]]; then echo "Unable to stop the process! Exiting!";  echo " - Nifi         - Failed [Unable to stop the process]" >> "$LOG_PATH"/SummaryReport.txt; exit $rc; fi
...
rc=$?; if [[ $rc != 0 ]]; then echo "Failed to post queue drop request! Exiting!";  echo " - Nifi         - Failed [Failed to post drop request]" >> "$LOG_PATH"/SummaryReport.txt; exit $rc; fi

sleep 5s

curl "${HEADER[@]}" -i -X DELETE "$NIFI_BASE_API_URL/connections/$compID?version=$compVersion"
rc=$?; if [[ $rc != 0 ]]; then echo "Connection couldn't be dropped! Exiting!";  echo " - Nifi         - Failed [Connection couldn't be dropped]" >> "$LOG_PATH"/SummaryReport.txt; exit $rc; fi

# Delete template
curl "${HEADER[@]}" -X DELETE "$NIFI_BASE_API_URL/templates/$templateID"
rc=$?; if [[ $rc != 0 ]]; then echo "Unable to stop the process! Exiting!";  echo " - Nifi         - Failed [Template couldn't be deleted from Nifi]" >> "$LOG_PATH"/SummaryReport.txt; exit $rc; fi
...
rc=$?; if [[ $rc != 0 ]]; then echo "Process group couldn't be deleted! Exiting!";  echo " - Nifi         - Failed [Process group couldn't be deleted]" >> "$LOG_PATH"/SummaryReport.txt; exit $rc; fi
